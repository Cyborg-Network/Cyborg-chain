...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(6)]
		#[pallet::weight({0})]
		pub fn ping(origin: OriginFor<T>, client: Vec<u8>) -> DispatchResult {
			// Check that the extrinsic was signed and get the signer.
			let _ = ensure_signed(origin)?;

			// Emit an event.
			Self::deposit_event(Event::Ping { client: String::from_utf8(client).unwrap() });
	...
	pub enum Event<T: Config> {
		...
		/// Event generated when a ping is sent to client
		Ping { client: String }
	}
